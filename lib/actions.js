...
module.exports = function(options) {
  ...
  function checkAccess(type, model, requestOptions, req, res, callback) {
    ...
    if (
      ...
      this.cb.call(this, this.requestOptions)
    } else {
      var self = this
      var actionAccessRule = options.accessRules[model.modelName][type]
      if (typeof actionAccessRule !== 'function') {
        if (actionAccessRule === '*') {
          self.cb.call(self, self.requestOptions)
        } else {
          util.logIfDebug('permission denied.')
          return res.send(403)
        }
      } else {
        actionAccessRule.call(model, req, res, self.requestOptions, function(allow, checkedOptions){
          if (!allow) {
            util.logIfDebug('permission denied.')
            return res.send(403)
          }
          self.cb.call(self, checkedOptions ? checkedOptions : self.requestOptions)
        })
      }
    }
  }
  ...
  return {
    ...
    delete: function(req, res) {
      ...
      model.findById(safeOptions.id, function(err, document) {
        if (err) {
          util.logIfDebug(err)
          return res.send(500, err)
        }

  ...
  return {
      ...
      checkAccess('list', model, requestOptions, req, res, function(safeOptions){
        ...
        _.each(safeOptions.query, function(v, k) {
          ...
          if (typeof query[k] === 'function') {
            if (k == 'populate' && typeof(v) === 'array') {
              for (var i = 0; i < v.length; i++) {
                query = query[k].apply(query, v[i]) 
              }
            } else {
              query = query[k].call(query, v) 
            }
          }
          return this
        })
    ...
    get: function(req, res) {
      ...
      checkAccess('get', model, requestOptions, req, res, function(safeOptions){
        ...
        queryParams = safeOptions.query
        query = model.findOne({_id: id, _destroy: {$ne: true}}, fields)
        pops = queryParams.populate ? queryParams.populate : queryParams.pop
        if (pops) {
          if (typeof(pops) === 'array') {
            for (var i = 0; i < pops.length; i++) {
              query.populate(pops[i])
            }
          } else {
            query.populate(pops)
          }
